...
def get_provider(request):
    ...
    if not number.isdigit() or len(number) > 11 or len(number) < 4:
        print('DEBUG get_provider: Number is not valid:', number)
        return JsonResponse({'error': 'Неправильный номер'}, status=400)
    if len(number) == 11 and (int(number[0]) == 7 or int(number[0]) == 8):
        number = number[1:]
    elif len(number) == 11:
        print('DEBUG get_provider: Number is not valid in russian format:', number)
    ...
    return JsonResponse({'provider': phone_provider.to_dict() if phone_provider else None})
